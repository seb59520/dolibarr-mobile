...
class _InvoiceFormPageState extends ConsumerState<InvoiceFormPage> {
  ...
  Future<void> _bootstrap(Invoice? existing) async {
  ...
  Future<bool?> _askRestoreDraft() => ConfirmDialog.show(
        ...
        message:
            'Vous avez des modifications non envoyées sur cette facture. '
            'Souhaitez-vous les reprendre ou repartir des valeurs serveur ?',
        confirmLabel: 'Reprendre',
        cancelLabel: 'Repartir',
      );

  void _hydrateFrom(Invoice i) {
    _refClientCtrl.text = i.refClient ?? '';
    _notePublicCtrl.text = i.notePublic ?? '';
    _notePrivateCtrl.text = i.notePrivate ?? '';
    _parentLocalId = i.socidLocal ?? _parentLocalId;
    _type = i.type;
    _dateInvoice = i.dateInvoice;
    _dateDue = i.dateDue;
  }
  ...
  void _hydrateFromDraft(Map<String, Object?> d) {
    ...
    final t = d['type'];
    if (t is int) _type = InvoiceType.fromInt(t);
    final di = d['date_invoice'];
    if (di is int) _dateInvoice = DateTime.fromMillisecondsSinceEpoch(di);
    final dd = d['date_due'];
    if (dd is int) _dateDue = DateTime.fromMillisecondsSinceEpoch(dd);
  }
  ...
  Future<void> _persistDraft() async {
    if (!mounted) return;
    await ref.read(invoiceRepositoryProvider).saveDraft(
          fields: _snapshot(),
          refLocalId: widget.existingLocalId,
        );
  }
  ...
  Map<String, Object?> _snapshot() => {
        ...
        'type': _type.apiValue,
        'date_invoice': _dateInvoice?.millisecondsSinceEpoch,
        'date_due': _dateDue?.millisecondsSinceEpoch,
      };
      ...
      if (widget.isCreate) {
        ...
        result.fold(
          onSuccess: (_) async {
            ...
            if (!mounted) return;
            _showSuccess(
              'Facture créée — ajoutez les lignes depuis la fiche.',
            );
            context.pop();
          },
  ...
  Future<Invoice?> _toEntity() async {
    ...
    return Invoice(
      ...
      type: _type,
      status: base?.status ?? InvoiceStatus.draft,
      paye: base?.paye ?? 0,
      dateInvoice: _dateInvoice,
      dateDue: _dateDue,
  ...
  Future<void> _confirmDiscard() async {
    ...
    await ref.read(invoiceRepositoryProvider).discardDraft(
          refLocalId: widget.existingLocalId,
        );
  ...
  Widget build(BuildContext context) {
    if (widget.existingLocalId != null) {
      final async = ref.watch(invoiceByIdProvider(widget.existingLocalId!));
      return async.when(
        data: (i) {
          if (!_initialized) {
            WidgetsBinding.instance
                .addPostFrameCallback((_) => _bootstrap(i));
          }
          return _scaffold(missingEntity: i == null);
        },
        loading: () => Scaffold(
                    ...
                    child: Form(
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          const _SectionTitle('Type & dates'),
  ...
  String _typeLabel(InvoiceType t) => switch (t) {
        InvoiceType.standard => 'Facture standard',
        InvoiceType.replacement => 'Remplacement',
        InvoiceType.creditNote => 'Avoir',
        InvoiceType.deposit => 'Acompte',
        InvoiceType.proforma => 'Pro forma',
        InvoiceType.situation => 'Situation',
      };
}
