...
/// Dialog modal de saisie d'une ligne de facture (création OU édition).
///
/// Retourne l'`InvoiceLine` modifiée, ou `null` si annulée. Le caller
/// est responsable d'appeler `createLocalLine` / `updateLocalLine`
/// sur le repository.
class InvoiceLineEditDialog extends StatefulWidget {
  const InvoiceLineEditDialog({
  ...
  final int invoiceLocalId;
  final int? invoiceRemoteId;
  final InvoiceLine? existing;

  static Future<InvoiceLine?> show(
    BuildContext context, {
    required int invoiceLocalId,
    int? invoiceRemoteId,
    InvoiceLine? existing,
  }) {
    return showDialog<InvoiceLine>(
      context: context,
      builder: (_) => InvoiceLineEditDialog(
  ...
  @override
  State<InvoiceLineEditDialog> createState() =>
      _InvoiceLineEditDialogState();
}

class _InvoiceLineEditDialogState extends State<InvoiceLineEditDialog> {
  ...
  InvoiceLineProductType _type = InvoiceLineProductType.service;
  int? _fkProduct;
  ...
  Future<void> _pickProduct() async {
    ...
    setState(() {
      ...
      _type = product.type == ProductType.service
          ? InvoiceLineProductType.service
          : InvoiceLineProductType.product;
    });
  }
  ...
  void _submit() {
    ...
    final entity = InvoiceLine(
      localId: base?.localId ?? 0,
      remoteId: base?.remoteId,
      invoiceLocal: widget.invoiceLocalId,
      invoiceRemote: widget.invoiceRemoteId ?? base?.invoiceRemote,
      ...
      content: SingleChildScrollView(
        child: Form(
          ...
          child: Column(
            ...
            children: [
              SegmentedButton<InvoiceLineProductType>(
                segments: const [
                  ButtonSegment(
                    value: InvoiceLineProductType.service,
                    label: Text('Service'),
                  ),
                  ButtonSegment(
                    value: InvoiceLineProductType.product,
                    label: Text('Produit'),
                  ),
                ],
                selected: {_type},
